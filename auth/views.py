    ...
    def post(self, request):
        ...
        if not User.objects.filter(username=username).exists():
            if not User.objects.filter(email=email).exists():
                if len(password) < 6:
                    messages.error(request, 'A senha deve possuir no mínimo 6 caracteres')
...
class ValidacaoUsuarioView(View):
    def post(self, request):
        ...
        if not str(username).isalnum():
            return JsonResponse({'error': 'O nome de usuário não pode conter caracteres especiais. Ex: (@,#,!... etc)'}, status=400)
        # checa se o usuário existe no banco de dados
        if User.objects.filter(username=username).exists():
            return JsonResponse({'error': 'O nome de usuário inserido já existe. Por favor escolha outro.'}, status=409)
        # else
...
class ValidacaoEmailView(View):
    def post(self, request):
        ...
        if not validate_email(email):
            return JsonResponse({'error_email': 'Insira um formato de email válido. Ex: username@email.com'}, status=400)
...
class LoginView(View):
    ...
    def post(self, request):
        ...
        if username and password:
            ...
            messages.error(request, 'Nome de usuário ou senha incorreta, por favor tente novamente')    
            return render(request, 'auth/login.html') 
